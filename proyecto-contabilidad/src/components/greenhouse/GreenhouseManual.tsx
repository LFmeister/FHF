'use client'

import { useState, type ReactNode } from 'react'
import {
  CheckCircle2,
  Cpu,
  Database,
  FileText,
  Gauge,
  GitBranch,
  HardDriveUpload,
  KeyRound,
  Lock,
  RadioTower,
  ShieldCheck,
  Smartphone,
  TerminalSquare,
  Wrench,
} from 'lucide-react'
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/Card'
import { useLanguage } from '@/context/LanguageContext'

function Pill({ children }: { children: ReactNode }) {
  return (
    <span className="inline-flex items-center rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-700">
      {children}
    </span>
  )
}

function ManualSection({
  icon,
  title,
  children,
}: {
  icon: ReactNode
  title: string
  children: ReactNode
}) {
  return (
    <section className="rounded-2xl border border-slate-200 bg-slate-50/80 p-5">
      <div className="mb-3 flex items-center gap-2">
        <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-white text-primary-700 ring-1 ring-slate-200">
          {icon}
        </span>
        <h4 className="text-base font-bold text-slate-900">{title}</h4>
      </div>
      {children}
    </section>
  )
}

function DiagramNode({
  x,
  y,
  label,
  sublabel,
  tone = 'slate',
}: {
  x: number
  y: number
  label: string
  sublabel: string
  tone?: 'emerald' | 'sky' | 'amber' | 'slate'
}) {
  const fill = {
    emerald: '#ecfdf5',
    sky: '#f0f9ff',
    amber: '#fffbeb',
    slate: '#f8fafc',
  }[tone]
  const stroke = {
    emerald: '#34d399',
    sky: '#38bdf8',
    amber: '#f59e0b',
    slate: '#cbd5e1',
  }[tone]

  return (
    <g>
      <rect x={x} y={y} width="150" height="76" rx="16" fill={fill} stroke={stroke} strokeWidth="2" />
      <text x={x + 75} y={y + 32} textAnchor="middle" className="fill-slate-900 text-[13px] font-bold">
        {label}
      </text>
      <text x={x + 75} y={y + 52} textAnchor="middle" className="fill-slate-500 text-[10px]">
        {sublabel}
      </text>
    </g>
  )
}

function Arrow({ x1, y1, x2, y2 }: { x1: number; y1: number; x2: number; y2: number }) {
  return (
    <line
      x1={x1}
      y1={y1}
      x2={x2}
      y2={y2}
      stroke="#64748b"
      strokeWidth="2"
      strokeLinecap="round"
      markerEnd="url(#arrow)"
    />
  )
}

function ArchitectureDiagram() {
  const { t } = useLanguage()
  const m = t.greenhouse.manual
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
      <svg viewBox="0 0 760 250" role="img" aria-label={m.diagramAriaArch} className="h-auto w-full">
        <defs>
          <marker id="arrow" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto" markerUnits="strokeWidth">
            <path d="M0,0 L0,6 L9,3 z" fill="#64748b" />
          </marker>
        </defs>
        <DiagramNode x={30} y={40} label={m.diagramSensors} sublabel={m.diagramSensorsSub} tone="emerald" />
        <DiagramNode x={220} y={40} label="ESP32" sublabel={m.diagramReadControl} tone="sky" />
        <DiagramNode x={410} y={40} label="Supabase" sublabel={m.diagramDataQueue} tone="amber" />
        <DiagramNode x={600} y={40} label="Dashboard" sublabel={m.diagramDashboardSub} tone="slate" />
        <DiagramNode x={220} y={150} label={m.diagramActuators} sublabel={m.diagramActuatorsSub} tone="emerald" />
        <DiagramNode x={410} y={150} label="Telegram AI" sublabel={m.diagramRemoteOp} tone="sky" />
        <Arrow x1={180} y1={78} x2={220} y2={78} />
        <Arrow x1={370} y1={78} x2={410} y2={78} />
        <Arrow x1={560} y1={78} x2={600} y2={78} />
        <Arrow x1={295} y1={116} x2={295} y2={150} />
        <Arrow x1={485} y1={150} x2={485} y2={116} />
        <path d="M600 178 C560 214 460 214 410 188" fill="none" stroke="#64748b" strokeWidth="2" markerEnd="url(#arrow)" />
      </svg>
    </div>
  )
}

function DataFlowDiagram() {
  const { t } = useLanguage()
  const m = t.greenhouse.manual
  const flowLabels = [m.diagramFlowRead, m.diagramFlowStore, m.diagramFlowShow, m.diagramFlowOrder]
  const lifeLabels = [m.diagramLifePending, m.diagramLifeTaken, m.diagramLifeExecuted, m.diagramLifeAck]
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
      <svg viewBox="0 0 760 210" role="img" aria-label={m.diagramAriaFlow} className="h-auto w-full">
        <defs>
          <marker id="arrow-flow" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto" markerUnits="strokeWidth">
            <path d="M0,0 L0,6 L9,3 z" fill="#0f766e" />
          </marker>
        </defs>
        <path d="M110 78 H650" stroke="#0f766e" strokeWidth="3" strokeLinecap="round" markerEnd="url(#arrow-flow)" />
        <path d="M650 145 H110" stroke="#0f766e" strokeWidth="3" strokeLinecap="round" markerEnd="url(#arrow-flow)" />
        {flowLabels.map((label, index) => (
          <g key={label}>
            <circle cx={110 + index * 180} cy="78" r="28" fill="#ccfbf1" stroke="#14b8a6" strokeWidth="2" />
            <text x={110 + index * 180} y="83" textAnchor="middle" className="fill-slate-900 text-[12px] font-bold">
              {label}
            </text>
          </g>
        ))}
        {lifeLabels.map((label, index) => (
          <g key={label}>
            <rect x={82 + index * 180} y="122" width="58" height="46" rx="12" fill="#f8fafc" stroke="#cbd5e1" />
            <text x={111 + index * 180} y="150" textAnchor="middle" className="fill-slate-700 text-[11px] font-semibold">
              {label}
            </text>
          </g>
        ))}
        <text x="380" y="28" textAnchor="middle" className="fill-slate-900 text-[15px] font-bold">
          {m.diagramFlowTitle}
        </text>
        <text x="380" y="195" textAnchor="middle" className="fill-slate-500 text-[12px]">
          {m.diagramFlowFooter}
        </text>
      </svg>
    </div>
  )
}

function SafetyDiagram() {
  const { t } = useLanguage()
  const m = t.greenhouse.manual
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
      <svg viewBox="0 0 760 250" role="img" aria-label={m.diagramAriaSafety} className="h-auto w-full">
        <defs>
          <marker id="arrow-safe" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto" markerUnits="strokeWidth">
            <path d="M0,0 L0,6 L9,3 z" fill="#be123c" />
          </marker>
        </defs>
        <rect x="36" y="38" width="170" height="70" rx="16" fill="#fff1f2" stroke="#fb7185" strokeWidth="2" />
        <text x="121" y="68" textAnchor="middle" className="fill-slate-900 text-[13px] font-bold">
          {m.diagramSafetyPumpOn}
        </text>
        <text x="121" y="90" textAnchor="middle" className="fill-slate-500 text-[10px]">
          {m.diagramSafetyCounter}
        </text>
        <rect x="294" y="38" width="170" height="70" rx="16" fill="#fffbeb" stroke="#f59e0b" strokeWidth="2" />
        <text x="379" y="68" textAnchor="middle" className="fill-slate-900 text-[13px] font-bold">
          {m.diagramSafety15min}
        </text>
        <text x="379" y="90" textAnchor="middle" className="fill-slate-500 text-[10px]">
          {m.diagramSafetyAuto}
        </text>
        <rect x="552" y="38" width="170" height="70" rx="16" fill="#fef2f2" stroke="#ef4444" strokeWidth="2" />
        <text x="637" y="68" textAnchor="middle" className="fill-slate-900 text-[13px] font-bold">
          {m.diagramSafetyLock}
        </text>
        <text x="637" y="90" textAnchor="middle" className="fill-slate-500 text-[10px]">
          {m.diagramSafetyFillBlocked}
        </text>
        <line x1="206" y1="73" x2="294" y2="73" stroke="#be123c" strokeWidth="2" markerEnd="url(#arrow-safe)" />
        <line x1="464" y1="73" x2="552" y2="73" stroke="#be123c" strokeWidth="2" markerEnd="url(#arrow-safe)" />
        <path d="M637 108 V154 C637 182 594 190 550 190 H214 C170 190 121 178 121 126 V108" fill="none" stroke="#be123c" strokeWidth="2" markerEnd="url(#arrow-safe)" />
        <text x="380" y="184" textAnchor="middle" className="fill-rose-700 text-[12px] font-bold">
          {m.diagramSafetyForce}
        </text>
        <rect x="238" y="203" width="284" height="28" rx="14" fill="#f0fdf4" stroke="#22c55e" />
        <text x="380" y="222" textAnchor="middle" className="fill-emerald-800 text-[12px] font-semibold">
          {m.diagramSafetyOta}
        </text>
      </svg>
    </div>
  )
}

function GeneralManualContent() {
  const { t } = useLanguage()
  const m = t.greenhouse.manual
  return (
    <div className="space-y-6">
      <div className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
        <ManualSection icon={<Gauge className="h-5 w-5" />} title={m.overviewTitle}>
          <p className="text-sm leading-6 text-slate-600">
            {m.overviewText}
          </p>
          <div className="mt-4 grid gap-2">
            {(m.systemSteps as readonly string[]).map((step, index) => (
              <div key={index} className="flex gap-3 rounded-xl bg-white px-3 py-2 text-sm text-slate-700 ring-1 ring-slate-200">
                <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-primary-100 text-xs font-bold text-primary-800">
                  {index + 1}
                </span>
                <span>{step}</span>
              </div>
            ))}
          </div>
        </ManualSection>

        <ArchitectureDiagram />
      </div>

      <div className="grid gap-4 xl:grid-cols-3">
        <ManualSection icon={<Cpu className="h-5 w-5" />} title={m.hardwareTitle}>
          <ul className="space-y-2 text-sm leading-6 text-slate-600">
            {(m.hardware as readonly string[]).map((item, index) => {
              const [bold, ...rest] = item.split(':')
              return (
                <li key={index}><b>{bold}:</b>{rest.join(':')}</li>
              )
            })}
          </ul>
        </ManualSection>

        <ManualSection icon={<Database className="h-5 w-5" />} title={m.dataTitle}>
          <ul className="space-y-2 text-sm leading-6 text-slate-600">
            {(m.dataItems as readonly string[]).map((item, index) => {
              const [bold, ...rest] = item.split(':')
              return (
                <li key={index}><b>{bold}:</b>{rest.join(':')}</li>
              )
            })}
          </ul>
        </ManualSection>

        <ManualSection icon={<Smartphone className="h-5 w-5" />} title={m.interfacesTitle}>
          <ul className="space-y-2 text-sm leading-6 text-slate-600">
            {(m.interfaces as readonly string[]).map((item, index) => {
              const [bold, ...rest] = item.split(':')
              return (
                <li key={index}><b>{bold}:</b>{rest.join(':')}</li>
              )
            })}
          </ul>
        </ManualSection>
      </div>

      <div className="grid gap-4 xl:grid-cols-[0.95fr_1.05fr]">
        <DataFlowDiagram />
        <ManualSection icon={<RadioTower className="h-5 w-5" />} title={m.cycleTitle}>
          <div className="space-y-3 text-sm leading-6 text-slate-600">
            <p>{m.cycleText}</p>
            <div className="grid gap-2 sm:grid-cols-2">
              {(m.commandLifecycle as readonly (readonly string[])[]).map(([state, detail]) => (
                <div key={state} className="rounded-xl bg-white p-3 ring-1 ring-slate-200">
                  <p className="font-mono text-xs font-bold uppercase text-slate-900">{state}</p>
                  <p className="mt-1 text-xs text-slate-600">{detail}</p>
                </div>
              ))}
            </div>
          </div>
        </ManualSection>
      </div>

      <div className="grid gap-4 xl:grid-cols-[1.05fr_0.95fr]">
        <ManualSection icon={<ShieldCheck className="h-5 w-5" />} title={m.safetyTitle}>
          <div className="space-y-3 text-sm leading-6 text-slate-600">
            <p>{m.safetyText}</p>
            <div className="grid gap-2">
              {(m.operatorChecks as readonly string[]).map((check, index) => (
                <div key={index} className="flex gap-2 rounded-xl bg-white px-3 py-2 ring-1 ring-slate-200">
                  <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" />
                  <span>{check}</span>
                </div>
              ))}
            </div>
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-amber-900">
              <b>{m.criticalRule}</b> {m.criticalRuleText}
            </div>
          </div>
        </ManualSection>

        <SafetyDiagram />
      </div>
    </div>
  )
}

function TechnicalManualContent() {
  const { t } = useLanguage()
  const m = t.greenhouse.manual

  const developerFilePaths = [
    'src/components/greenhouse/GreenhouseTab.tsx',
    'src/lib/greenhouse.ts',
    'supabase/greenhouse_*.sql',
    'agents/meister_greenhouse_bot.py',
    'arduinoV1/arduino/esp32_greenhouse_supabase',
  ]

  const payloadFieldKeys = [
    'status.pump_on',
    'status.pump_remaining_ms',
    'status.pump_safety_lock',
    'float_switches.low/mid/high',
    'dht_sensors[]',
    'soil_sensors[]',
    'esp32.firmware_version',
  ]

  return (
    <div className="space-y-6">
      <div className="grid gap-4 xl:grid-cols-2">
        <ManualSection icon={<HardDriveUpload className="h-5 w-5" />} title={m.otaTitle}>
          <ol className="space-y-2 text-sm leading-6 text-slate-600">
            {(m.otaSteps as readonly string[]).map((step, index) => (
              <li key={index}>{step}</li>
            ))}
          </ol>
          <p className="mt-3 rounded-xl bg-white p-3 text-xs leading-5 text-slate-600 ring-1 ring-slate-200">
            {m.otaNote}
          </p>
        </ManualSection>

        <ManualSection icon={<KeyRound className="h-5 w-5" />} title={m.credsTitle}>
          <ul className="space-y-2 text-sm leading-6 text-slate-600">
            {(m.creds as readonly string[]).map((item, index) => {
              const [bold, ...rest] = item.split(':')
              return (
                <li key={index}><b>{bold}:</b>{rest.join(':')}</li>
              )
            })}
          </ul>
          <p className="mt-3 rounded-xl border border-rose-200 bg-rose-50 p-3 text-xs leading-5 text-rose-800">
            {m.credsWarning}
          </p>
        </ManualSection>
      </div>

      <ManualSection icon={<TerminalSquare className="h-5 w-5" />} title={m.techRefTitle}>
        <div className="grid gap-4 xl:grid-cols-[1fr_1fr]">
          <div>
            <h5 className="font-semibold text-slate-900">{m.importantFiles}</h5>
            <div className="mt-3 space-y-2">
              {developerFilePaths.map((path, index) => (
                <div key={path} className="rounded-xl bg-white p-3 ring-1 ring-slate-200">
                  <p className="font-mono text-xs font-bold text-slate-900">{path}</p>
                  <p className="mt-1 text-xs leading-5 text-slate-600">{(m.developerFileDetails as readonly string[])[index]}</p>
                </div>
              ))}
            </div>
          </div>

          <div>
            <h5 className="font-semibold text-slate-900">{m.payloadFields}</h5>
            <div className="mt-3 overflow-hidden rounded-xl border border-slate-200 bg-white">
              <table className="w-full text-left text-xs">
                <tbody>
                  {payloadFieldKeys.map((field, index) => (
                    <tr key={field} className="border-b border-slate-100 last:border-0">
                      <td className="w-[42%] px-3 py-2 font-mono font-semibold text-slate-900">{field}</td>
                      <td className="px-3 py-2 leading-5 text-slate-600">{(m.payloadFieldDetails as readonly string[])[index]}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>

            <h5 className="mt-5 font-semibold text-slate-900">{m.recognizedCommands}</h5>
            <div className="mt-3 flex flex-wrap gap-2">
              {['STATUS', 'FILL ON', 'FILL OFF', 'FILL_FORCE_ON', 'PUMP_UNLOCK', 'SET_PUMP <segundos>', 'LIGHT_ON', 'LIGHT_OFF'].map((command) => (
                <Pill key={command}>{command}</Pill>
              ))}
            </div>
          </div>
        </div>
      </ManualSection>

      <div className="grid gap-4 md:grid-cols-3">
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
          <Wrench className="h-5 w-5 text-emerald-700" />
          <h4 className="mt-3 font-bold text-emerald-950">{m.maintenanceTitle}</h4>
          <p className="mt-2 text-sm leading-6 text-emerald-900">
            {m.maintenanceText}
          </p>
        </div>
        <div className="rounded-2xl border border-sky-200 bg-sky-50 p-4">
          <GitBranch className="h-5 w-5 text-sky-700" />
          <h4 className="mt-3 font-bold text-sky-950">{m.versioningTitle}</h4>
          <p className="mt-2 text-sm leading-6 text-sky-900">
            {m.versioningText}
          </p>
        </div>
        <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4">
          <Lock className="h-5 w-5 text-rose-700" />
          <h4 className="mt-3 font-bold text-rose-950">{m.failuresTitle}</h4>
          <p className="mt-2 text-sm leading-6 text-rose-900">
            {m.failuresText}
          </p>
        </div>
      </div>
    </div>
  )
}

export function GreenhouseManual() {
  const { t } = useLanguage()
  const m = t.greenhouse.manual
  const [selectedManual, setSelectedManual] = useState<'general' | 'technical' | null>(null)

  return (
    <Card id="manual-invernadero" className="bg-white/90">
      <CardHeader>
        <div className="flex flex-wrap items-center gap-2">
          <Pill>
            <FileText className="mr-2 h-3.5 w-3.5 text-primary-700" />
            {m.hiddenManual}
          </Pill>
          <Pill>
            <TerminalSquare className="mr-2 h-3.5 w-3.5 text-slate-600" />
            {m.twoVersions}
          </Pill>
        </div>
        <CardTitle className="text-slate-900">{m.title}</CardTitle>
        <CardDescription>
          {m.subtitle}
        </CardDescription>
      </CardHeader>

      <CardContent className="space-y-6">
        <div className="grid gap-3 sm:grid-cols-2">
          <button
            type="button"
            onClick={() => setSelectedManual(selectedManual === 'general' ? null : 'general')}
            className={`rounded-2xl border p-5 text-left transition ${
              selectedManual === 'general'
                ? 'border-emerald-300 bg-emerald-50 shadow-[0_12px_28px_rgba(16,185,129,0.14)]'
                : 'border-slate-200 bg-slate-50 hover:border-emerald-300 hover:bg-emerald-50/70'
            }`}
            aria-expanded={selectedManual === 'general'}
            aria-controls="manual-general"
          >
            <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-white text-emerald-700 ring-1 ring-emerald-200">
              <Gauge className="h-5 w-5" />
            </span>
            <span className="mt-4 block text-lg font-bold text-slate-900">{m.generalVersion}</span>
            <span className="mt-1 block text-sm leading-6 text-slate-600">
              {m.generalDesc}
            </span>
          </button>

          <button
            type="button"
            onClick={() => setSelectedManual(selectedManual === 'technical' ? null : 'technical')}
            className={`rounded-2xl border p-5 text-left transition ${
              selectedManual === 'technical'
                ? 'border-sky-300 bg-sky-50 shadow-[0_12px_28px_rgba(14,165,233,0.14)]'
                : 'border-slate-200 bg-slate-50 hover:border-sky-300 hover:bg-sky-50/70'
            }`}
            aria-expanded={selectedManual === 'technical'}
            aria-controls="manual-tecnico"
          >
            <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-white text-sky-700 ring-1 ring-sky-200">
              <TerminalSquare className="h-5 w-5" />
            </span>
            <span className="mt-4 block text-lg font-bold text-slate-900">{m.technicalVersion}</span>
            <span className="mt-1 block text-sm leading-6 text-slate-600">
              {m.technicalDesc}
            </span>
          </button>
        </div>

        {!selectedManual && (
          <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-5 py-8 text-center">
            <FileText className="mx-auto h-8 w-8 text-slate-400" />
            <p className="mt-3 text-sm font-semibold text-slate-700">
              {m.hiddenUntilSelect}
            </p>
          </div>
        )}

        {selectedManual === 'general' && (
          <div id="manual-general">
            <GeneralManualContent />
          </div>
        )}

        {selectedManual === 'technical' && (
          <div id="manual-tecnico">
            <TechnicalManualContent />
          </div>
        )}
      </CardContent>
    </Card>
  )
}
